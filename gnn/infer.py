...
def gpu_train(GPUid, DATA_PATH,
    ...
    # train_nid = np.concatenate((train_nid,val_nid),axis=0)
    graph = graph.int()
    ...
    if gnn_model == 'graphsage':
        ...
        model = GraphAttnModel(in_feat, hidden_dim, n_layers, n_classes,
                               heads=([6] * n_layers), activation=F.leaky_relu, feat_drop=0, attn_drop=0)
    else:
    ...
    model.load_state_dict(th.load("../save/graphattn-Epoch_6.pth"))
    model = model.to(device_id)
    ...
    with th.no_grad():
        for step, (input_nodes, seeds, blocks) in enumerate(test_dataloader):
            ...
            j=0
            for i in seeds:
                if lineCount > 591973 : break
                csv_writer.writerow([nodes_df.loc[int(i)].paper_id, chr(65+test_batch_pred[j])])
                j+=1
                lineCount += 1
                if lineCount%100000 == 0: print("lineCount:", lineCount)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_dim', type=int, required=False, default=96)
    parser.add_argument('--n_layers', type=int, default=3)
    parser.add_argument("--fanout", type=str, required=False, help="fanout numbers", default='30,30,30')
