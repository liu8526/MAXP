...
class GraphAttnModel(thnn.Module):
    ...
    def __init__(self,
        ...
        self.linelayers = thnn.Linear(self.in_feats, self.hidden_dim)
        self.feat_drop = thnn.Dropout(self.feat_dropout)
    ...
    def forward(self, blocks, features):
        ...
        h = self.activation(self.linelayers(h))

        res_feature = []
        for l in range(self.n_layers - 1):
            res_feature.append(h[blocks[l].dstnodes().to(th.long)])

        for l in range(self.n_layers - 1):            
            h = self.layers[l](blocks[l], h)
            for i in  range(h.shape[1]):
                 h[:, i, :] = res_feature[l] + h[:, i, :]
            h = h.flatten(1)

        logits = self.layers[-1](blocks[-1],h)
